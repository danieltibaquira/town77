...
export function placeChipHandler(io: Io, socket: Sock, db: Db) {
  return (payload: PlaceChipPayload) => {
    ...
    try {
      ...
      const chip_idx = currentPlayer.hand.findIndex(
      ...
      const isValid = isValidPlacement(state.grid, row, col, chip, isFirst)
      if (!isValid) {
        logger.warn({
          roomCode,
          playerId,
      ...
      const handWithout = [
        ...currentPlayer.hand.slice(0, chip_idx),
        ...currentPlayer.hand.slice(chip_idx + 1),
      ]
