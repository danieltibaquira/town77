...
describe('Cafe Queue full game', () => {
  ...
  it('persists the final-round boundary and emits server-calculated results', async () => {
    ...
    await Promise.all([started, startedGuest])
    ...
    expect(afterGuest.turnIndex).toBe(0)
    ...
    await finalGuest
    expect((await gameOver).scores.map((score) => score.playerId)).toEqual([joined.playerId, expect.any(String)])
