...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  connect: () => {
    ...
    socket.on("connect", () => {
      set({ connected: true });
      // socket.io room membership is in-memory and lost on every transport
      // reconnect (wifi drop) or server restart/redeploy. The transport
      // auto-reconnects but the server places this new socket in no room, so
      // presence broadcasts (state_update) silently stop reaching us until a
      // manual reload. Re-join on (re)connection so the server re-adds us to
      // the room via its session-recovery path.
      const { roomCode, sessionToken, playerId } = get();
      if (roomCode && sessionToken && playerId) {
        const playerName =
          (typeof localStorage !== "undefined" && localStorage.getItem("playerName")?.trim()) || "";
        get().joinRoom(roomCode, playerName, playerId, sessionToken);
      }
    });
