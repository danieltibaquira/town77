  ...
  return (
    <main data-testid="game-screen" style={{
      ...
      <header style={{
        ...
        <span key={bagShakeKey} data-testid="bag-count" style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--text-sm)', marginLeft: 'auto', ...bagShake }}>
          {gameState.bag.length}
        </span>
      </header>

      <div key={isMyTurn ? 'my-turn' : 'waiting'} data-testid="turn-indicator" style={{ color: isMyTurn ? 'var(--color-text-accent)' : 'var(--color-text-secondary)', fontSize: 'var(--text-lg)', fontWeight: isMyTurn ? 700 : 400, padding: 'var(--space-sm) var(--space-md)', textAlign: 'center', letterSpacing: '0.05em', ...turnSweep }}>
        {isMyTurn ? t('your_turn') : t('waiting')}
      </div>

      <div data-testid="game-panel" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)', padding: 'var(--space-md)', contain: 'var(--layout-contain)', flex: 1, justifyContent: 'center' }}>
        ...
        <div style={{ padding: 'var(--space-sm) var(--space-md)', position: 'relative', maxWidth: 600, margin: '0 auto', width: '100%' }}>
          <Hand chips={myPlayer?.hand ?? []} selectedChip={selectedChip} onSelect={isMyTurn ? selectChip : () => {}} />
          {/* T10: Exchange flash overlay */}
          <div key={flashKey} style={{ position: 'absolute', inset: 0, borderRadius: 'var(--radius-md)', pointerEvents: 'none', ...exchangeFlash }} />
        </div>
