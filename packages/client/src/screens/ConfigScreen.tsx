    ...
    >
      ...
      <div style={{ display: 'grid', gap: 'var(--space-sm)', gridTemplateColumns: '1fr 1fr 1fr' }}>
        {THEMES.map((themeItem) => (
          <ThemeCard
            ...
            onClick={() => setSelectedThemeId(themeItem.id as ThemeId)}
          />
        ))}
      </div>

      <div data-testid="config-grid">
        <Stepper label={t('grid_size')} value={gridSize} min={3} max={11} onChange={setGridSize} />
      </div>
      <div data-testid="config-colors">
        <Stepper label={t('colors')} value={colors} min={2} max={7} onChange={setColors} />
      </div>
      <div data-testid="config-shapes">
        <Stepper label={t('shapes')} value={shapes} min={2} max={7} onChange={setShapes} />
      </div>
      <div data-testid="config-copies">
        <Stepper label={t('copies')} value={copies} min={1} max={3} onChange={setCopies} />
      </div>
      <div data-testid="config-hand-size">
        <Stepper label={t('hand_size')} value={handSize} min={3} max={6} onChange={setHandSize} />
      </div>
      ...
      {showWarning && (
        <div
          ...
          style={{ color: 'var(--color-text-accent)', fontSize: 'var(--text-sm)' }}
        >
        ...
        style={{
          ...
          boxShadow:
            canCreate && isNeo
              ? `${theme.styleProps.shadowOffset}px ${theme.styleProps.shadowOffset}px 0px ${theme.styleProps.shadowColor}`
              : canCreate
                ? 'var(--shadow-md), 0 0 12px rgba(196, 163, 90, 0.2)'
                : 'none',
        }}
      >
        {tc('create_room')}
      ...
      <button
        ...
        style={{
          background: canCreate
            ? isNeo
              ? '#4ecdc4'
              : 'linear-gradient(135deg, rgba(196, 163, 90, 0.15) 0%, rgba(196, 163, 90, 0.05) 100%)'
            : 'var(--color-surface-cell)',
          border: isNeo
            ? `${theme.styleProps.borderWidth}px solid ${theme.styleProps.borderColor}`
