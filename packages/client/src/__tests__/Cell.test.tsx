...
describe('Cell', () => {
  ...
  it('renders chip when occupied', () => {
    renderWithTheme(
      <Cell row={3} col={4} chip={{ color: 'color-2', shape: 'tower' }} isValid={false} />,
    )
  ...
  describe('Surface Depth', () => {
    ...
    it('does not apply inner shadow to occupied cells', () => {
      renderWithTheme(
        <Cell row={1} col={1} chip={{ color: 'color-1', shape: 'barn' }} isValid={false} />,
      )
      const cell = screen.getByTestId('cell-1-1')
      const style = cell.getAttribute('style') || ''
  ...
  describe('Placement Ripple', () => {
    it('applies placement-ripple animation when occupied', () => {
      renderWithTheme(
        <Cell row={0} col={0} chip={{ color: 'color-1', shape: 'cottage' }} isValid={false} />,
      )
